...
def _reset_gis():
    for key in ("gdf_gis", "avisos_gis", "gdf_contorno"):
...
def render_sidebar() -> dict:
    st.sidebar.image(
        "https://img.icons8.com/color/96/sugarcane.png",
        width=60,
    )
    ...
    with st.sidebar.expander("🌾 Produção Agronômica", expanded=False):
        ...
        preco_tonelada  = st.number_input("Preço da tonelada (R$/t)",           value=140.0, min_value=1.0, step=5.0,  format="%.2f")
    ...
    return dict(
        ...
        ganho_esperado_tha=ganho_esperado_tha, preco_tonelada=preco_tonelada,
        taxa_pegamento=taxa_pegamento, custo_muda_tha=custo_muda_tha,
...
Linhas com **IOI ≥ IOI mínimo** são exportadas para o piloto automático.
    ...
    # ── Custo por hora ────────────────────────────────────────────────────
    with st.expander(f"💰 Custo da máquina: R$ {custo_h:.2f}/hora", expanded=False):
    ...
    with tab_talhao:
        render_por_talhao(gdf_linhas_eco, gdf_gis, params["espacamento"])
